...
start, answer RPS/TP, then play. Requires that turn count advances and/or
LP changes -- proof the game actually plays, not just clicks."""
...
try:
    ...
    for _ in range(20):
        ...
        time.sleep(0.5)
        if _ in (0, 5):
            print("wait-start", repr((t or "")[:100]))
    if not started:
        ...
        for m, v in ((a, 1), (b, 2)):
            ...
            if "RPS choice" in t:
                ...
                if "pass" in t or "summon" in t or "attack" in t or "activate" in t or "set-" in t or "reposition" in t:
                    ...
                    m.choose(id=0)
            elif "Game Over" in t or "Match Over" in t:
...
print("samples:", len(samples), "max turn:", max_turn)
print("LP0 seen:", sorted(lp0s)[:8], "LP1 seen:", sorted(lp1s)[:8])
ok = max_turn >= 2 or (len(lp0s) > 1 or len(lp1s) > 1)
